...
Status: Static snapshot format and bounded CLI tooling implemented in alpha; broad ecosystem adoption is not a stable-v1 completion gate.
...
The Directory should be treated as an optional index over OrgAnchor Beacons. `docs/protocol/ORGANCHOR_BEACON.md` defines the origin-owned signals that let adopters be found even before a Directory includes them.
...
The current alpha format version is:
