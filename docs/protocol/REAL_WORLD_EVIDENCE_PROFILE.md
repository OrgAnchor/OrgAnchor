...
See `docs/protocol/EVIDENCE_SUFFICIENCY_MODEL.md`.

The broader integration of purpose profiles, observation source classes, and challenge/correction lifecycle is defined in `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md`.
...
For real-world products and services, support should expose the source class behind each evidence item or observation, using the canonical S1-S5 definitions in `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md`.
...
When a concrete product, batch, unit, service delivery, or project is involved, S3-S5 observations should bind to product/service credentials as described in `docs/protocol/PRODUCT_SERVICE_CREDENTIAL_LAYER.md`.

All real-world claims and evidence should follow `docs/protocol/SUBJECT_BINDING_MODEL.md`: discovery, claims, and evidence can use different granularities, but the evidence must not be widened beyond its declared subject.
