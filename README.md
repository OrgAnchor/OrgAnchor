...
This source release is `organchor@0.1.0-alpha.5`. Alpha releases are published through the matching Git tag and GitHub Trusted Publishing workflow under the npm `alpha` dist-tag; check `npm view organchor@alpha version` for authoritative registry state. The immediately preceding public baseline was Alpha.4, whose source, npm package, GitHub Release, homepage, `/verify` page, Beacon, machine index, and installed-CLI path were publicly rechecked on 2026-07-15. The aligned public self-pilot remains at `https://organchor.org`. Fireseed Alpha has a GO decision for named early adopter, technical, Agent, and evidence/governance review. It is not stable v1, and broader promotion remains subject to the current release gate. The core alpha surface includes root authority records, signed official-presence records, claims/evidence manifests, value continuity reports, carrier receipts, domain audit, Onion/ENS support, OpenTimestamps anchoring, root authority migration, AI-agent verification discovery, policy-route hints, and `/verify` root continuity publication.
