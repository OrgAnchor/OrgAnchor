...
Raw evidence health follows `docs/protocol/EVIDENCE_RETENTION_REALITY_PRINCIPLE.md`. A package or Directory should expose when S3-S5 raw bundles are available, request-restricted, restricted, summary-only, expired, withdrawn, lost, or disputed. Missing raw bundles are not protocol failures by themselves; hiding that condition is the failure.
