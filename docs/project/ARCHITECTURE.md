...
- Crawlers and AI agents can recognize OrgAnchor adopters from known origins,
  shared seeds, sitemaps, bounded crawl starts, or open Directory snapshots
  without depending on one official registry.
- Beacon publication does not by itself provide global discovery coverage.
...
Accepted future protocol artifacts, not current alpha outputs, include:
