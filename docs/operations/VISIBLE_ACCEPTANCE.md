...
如果目的是判断“一个候选组织今天答应后，我们是否已经具备接入
能力”，使用同一闭环的试点入口：
...
本地演习通过，只表示技术闭环已经就绪。它不等于公开 Level 2
试点已经完成。真实 Level 2 仍然需要组织确认文案和根权威方案、批准首次
公开发布、控制一个公开网站入口并完成域名安全审计。如果这些条件已经
具备，技术生成和验证可以在同一工作会话中完成；等待组织决策、DNS、
托管平台或人工审批的时间不能伪装成软件执行时间。
...
Fireseed Alpha 阶段，`docs/operations/VISIBLE_ACCEPTANCE.md` 是面向非开发者的第一层验收入口。
...
5. 再进入 `docs/evaluations/CAPABILITY_TRACEABILITY_MATRIX.md` 和 `docs/evaluations/CAPABILITY_AUDIT_SCENARIOS.md` 做更深的实现一致性审计。
