...
```text
The current OrgAnchor alpha can generate, sign, publish, and verify an adopting organization's endpoint statement, claims/evidence manifests, public /verify page, and AI-agent-readable verification index. Check `docs/project/IMPLEMENTATION_STATUS.md` and the installed package version for the exact current boundary.
```
