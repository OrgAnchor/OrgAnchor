...
It turns the S2 boundary from `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md` into fields, checks, and agent-facing summaries that can be implemented in schemas, `value audit`, `/verify`, and Directory health summaries.
...
S2 subject binding follows `docs/protocol/SUBJECT_BINDING_MODEL.md`. If S2 is used to support a concrete product, model, batch, unit, service, project, deployment, or dataset claim, `covered_subject_type`, `covered_subject_id`, and `scope_text` must make that coverage explicit. Otherwise the material may remain useful as broad S2, but it should not be treated as exact support for the concrete subject.
