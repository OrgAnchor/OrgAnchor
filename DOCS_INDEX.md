...
- `DIRECTORY_SNAPSHOT_SPEC.md`: proposed static Directory snapshot format, optional origin verification mode, and verify-index discovery pointer for low-cost AI-agent candidate discovery before direct origin verification.
...
- The discovery strategy and static Directory snapshot shape are documented, and static Directory build/verify, optional per-origin verification, and verify-index discovery pointers exist; broader crawling and ranking policy have not been implemented yet.
