...
Stages 1-5 functional scope: IMPLEMENTED_FOR_ALPHA_WITH_DISCLOSED_EXTERNAL_GAPS
S1-S3 evidence baseline: IMPLEMENTED_AND_TESTED
S4 real-use observation: PARTIAL / DESIGN_PREVIEW
...
The authoritative capability-by-capability status is
`docs/evaluations/CAPABILITY_TRACEABILITY_MATRIX.md`. A feature described in a
protocol design document is not shipped merely because its design is accepted.
...
- Public self-pilot release-state hashes and carrier receipts recorded in `docs/history/RELEASE_STATE_2026-05-25.md`.
- Public self-pilot minimal review recorded in `docs/history/PUBLIC_SELF_PILOT_MINIMAL_REVIEW_2026-07-06.md`, including public compact verification, doctor readiness, signed statement verification, signed claims/evidence verification, and root-signed lockfile verification.
- Alpha.4 public release linkage, clean npm install, homepage, `/verify`, Beacon, machine index, and installed-CLI verification recorded in `docs/history/PUBLIC_SELF_VERIFICATION_2026-07-15.md`.
...
The current priority after these three bounded scenarios is to preserve the
working public self-pilot, complete the first low-risk external organization
pilot, and use that pilot to test adoption friction, evidence preparation,
third-party Agent interpretation, and the gap between local discovery mechanics
and real-world candidate discovery. Additional synthetic scenarios are useful
only when they test a distinct failure mode or respond to an observed pilot gap.

The current launch path is Fireseed Alpha, defined in `docs/operations/FIRESEED_ALPHA_PLAN.md`, with the operational GO/HOLD decision in `docs/operations/FIRESEED_READINESS_GATE.md` and the first GO decision in `docs/history/FIRESEED_LAUNCH_DECISION_2026-06-01.md`.
...
5. Use `docs/history/PUBLIC_SELF_PILOT_MINIMAL_REVIEW_2026-07-06.md`, `docs/operations/PUBLIC_RELEASE_CHECKLIST.md`, and `docs/operations/FIRESEED_READINESS_GATE.md` as the public-release evidence gate.
6. Invite named early adopting organizations, technical reviewers, and evidence/governance critics according to `docs/history/FIRESEED_LAUNCH_DECISION_2026-06-01.md`.
7. Run the first low-risk external pilot using `docs/guides/PILOT_MINIMAL_PATH.md` first, then `docs/guides/EXTERNAL_PILOT_RUNBOOK.md` if the pilot needs the full path.
...
- Dedicated method, attestation, and challenge manifests from `docs/protocol/CLAIMS_EVIDENCE_PROTOCOL.md`.
- Delegated product/service key statements, product model passports, batch commitments, unit credentials, service delivery credentials, and observation binding checks from `docs/protocol/PRODUCT_SERVICE_CREDENTIAL_LAYER.md`.
- Commercial-fit manifests, signed public price sheets, delegated commercial keys, private signed quote verification, and compact commercial-fit summaries from `docs/protocol/COMMERCIAL_FIT_LAYER.md`.
- Common subject binding helpers, subject relation checks, and compact subject coverage outputs from `docs/protocol/SUBJECT_BINDING_MODEL.md`.
- Purpose-fit evidence outputs such as `fit_for`, `not_enough_for`, and `missing_optional_context` from `docs/protocol/EVIDENCE_SUFFICIENCY_MODEL.md`.
- Discovery Unit schemas, validators, Beacon fields, Directory fields, and coverage-preview query behavior from `docs/protocol/DISCOVERY_UNIT_MODEL.md`.
- P1-P5 purpose-profile validators, broader S1/S4/S5 source-class fields, S2 network verified-route adapters, institution-origin identity/reputation checks beyond the implemented same-origin hash-bound external signature route, S3 custody/independent-test route adapters from `docs/protocol/S3_RANDOM_SAMPLING_MODEL.md`, and S5 challenge/correction/accountability lifecycle extraction from `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md`.
...
- Package health commands, observed health lookup, Beacon health summaries, Directory health observation summaries, observed health reports, and agent fetch recommendations from `docs/protocol/PACKAGE_HEALTH_LAYER.md`.
