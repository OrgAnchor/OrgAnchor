...
OrgAnchor should also classify evidence and observations by source class, following the canonical S1-S5 definitions in `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md`.
...
S2 third-party material must follow the S2 boundary in `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md`: organization-submitted external-looking material without an external recheck anchor is `UNVERIFIED_EXTERNAL_MATERIAL`, not effective S2.
...
Purpose profiles and challenge/correction/accountability lifecycle are defined in `docs/protocol/PURPOSE_EVIDENCE_CHALLENGE_MODEL.md`. Public challenge review is not a sixth ascending purpose profile; it is a horizontal lifecycle mode that can apply to any purpose.
