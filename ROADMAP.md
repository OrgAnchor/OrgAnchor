...
The release-hygiene baseline has been merged into `main`. The current emphasis is MVP launch and repeatable adoption: keep `PROJECT_NORTH_STAR.md`, `README.md`, `DOCS_INDEX.md`, `CHANGELOG.md`, `V1_RELEASE_CHECKLIST.md`, public self-pilot state, npm package metadata, GitHub state, and carrier receipts aligned while preparing the next alpha release id and the first low-risk external pilot. Do not promote v1 stable until the package, public artifacts, and at least one external pilot are aligned.
