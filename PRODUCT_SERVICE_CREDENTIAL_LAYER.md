...
OrgAnchor should not treat all evidence sources as equivalent.
...
The accepted observation source ladder is:
...
The organization publishes its own specification sheets, production descriptions, internal test reports, manuals, logs, or operating records.

These are cheap and useful for accountability, but they remain self-issued unless strengthened by other sources.

### S2 Third-Party Documents

External labs, auditors, certifiers, inspection firms, platforms, regulators, or customers publish or sign relevant records.
...
The tested or observed product is selected from the market, warehouse, distributor inventory, or customer site rather than supplied as a hand-picked sample by the organization.
...
This source class directly addresses the "golden sample" problem.

### S4 Field-Use Observation

Real use creates observations from customers, repair providers, distributors, warranty events, returns, sensors, telemetry, service logs, and support records.

These observations are useful because they are generated by normal operations instead of staged proof exercises.

They should still expose:
...
The system must make counterexamples, failed samples, complaints, failed retests, contradictions, corrections, withdrawals, and unresolved disputes visible.
