...
test("S3 intake and slot model is package-facing and indexed", () => {
  ...
  const s3Model = readText("docs/protocol/S3_RANDOM_SAMPLING_MODEL.md");

  assert.ok(packageIncludes(packageJson.files, "docs/protocol/S3_INTAKE_AND_SLOT_MODEL.md"));
...
test("S3 intake model preserves anti-brushing and storage admission boundaries", () => {
  const model = readText("docs/protocol/S3_INTAKE_AND_SLOT_MODEL.md");
