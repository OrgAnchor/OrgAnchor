...
- `DIRECTORY_SNAPSHOT_SPEC.md`: proposed static Directory snapshot format and optional origin verification mode for low-cost AI-agent candidate discovery before direct origin verification.
...
- The discovery strategy and static Directory snapshot shape are documented, and static Directory build/verify plus optional per-origin verification exist; broader crawling, ranking policy, and a public Directory index have not been implemented yet.
