# Project Design And State

This directory contains OrgAnchor's purpose, architecture, accepted product
shape, current implementation state, roadmap, and project-level decisions.

## Current Sources Of Truth

- [PROJECT_NORTH_STAR.md](./PROJECT_NORTH_STAR.md): alignment rule for reducing
  discovery, verification, understanding, comparison, and decision friction.
- [IMPLEMENTATION_STATUS.md](./IMPLEMENTATION_STATUS.md): current implemented
  capability surface and known gaps.
- [ROADMAP.md](./ROADMAP.md): ordered future work.
- [V1_ACCEPTANCE.md](./V1_ACCEPTANCE.md): definition of v1 completeness.

## Design Rationale

- [DESIGN_RATIONALE.md](./DESIGN_RATIONALE.md)
- [ARCHITECTURE.md](./ARCHITECTURE.md)
- [PRODUCT_SHAPE.md](./PRODUCT_SHAPE.md)
- [PURPOSE_AND_VALUES.md](./PURPOSE_AND_VALUES.md)
- [THREAT_MODEL.md](./THREAT_MODEL.md)

## Decisions And Governance

- [AI_OPERATING_MODEL.md](./AI_OPERATING_MODEL.md)
- [TECHNICAL_DECISIONS.md](./TECHNICAL_DECISIONS.md)
- [CRYPTO_POLICY.md](./CRYPTO_POLICY.md)
- [ADOPTION_MODEL.md](./ADOPTION_MODEL.md)
- [PROJECT_BRIEF.md](./PROJECT_BRIEF.md)

When a dated historical document conflicts with this directory, use the current
sources of truth above and treat the dated document as a record of its own time.
