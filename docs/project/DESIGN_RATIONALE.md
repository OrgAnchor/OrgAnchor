...
Implementation state: implemented and tested in the current alpha.
...
Implementation state: implemented for known origins, supplied seeds, sitemaps,
bounded crawl starts, and local indexes; broad internet discovery coverage is
not implemented.
...
Implementation state: implemented and tested for direct and compact
verification; external Agent policy remains outside OrgAnchor.
...
Implementation state: S1-S3 baseline implemented and tested; S4 is partial and
design-preview; S5 remains design-only.
...
Implementation state: accepted design direction, not implemented as delegated
credential tooling in the current alpha.
...
Implementation state: profile-specific gap checks exist, but general
purpose-fit outputs and policy profiles remain incomplete.
...
Implementation state: local tooling and receipt verification are implemented;
real carrier availability, credentials, funding, and retention remain external.
...
Implementation state: implemented and tested, including threshold migration
verification and `/verify` continuity publication.
...
independent Directory adoption and broad ecosystem coverage are not.
...
Implementation state: accepted design direction, not implemented as signed
commercial-fit manifests or quote verification in the current alpha.
...
| Declared current official presence | Endpoints declared as current are signed by the authority; availability and real-world control still require checking. |
...
For schema and protocol evolution, the project follows `docs/protocol/PROTOCOL_EVOLUTION_POLICY.md`: new versions can add stronger checks and clearer gap visibility, but historical adopter packages must remain verifiable under their original version-specific rules when those rules are supported.
...
The final trust decision remains external. The design can reduce repeated
retrieval and mechanical verification work when organizations publish complete,
fresh packages and agents understand the contract.

OrgAnchor has not yet demonstrated broad, population-level transaction-cost
reduction across independent organizations and Agent implementations. That
outcome remains an external-pilot and ecosystem validation question.
